    ...
    while true do
        ...
        if dst <= 1 then
            ...
            if not isProcessing then
                DrawText3D( Config.kordinatlar.aseton.x, Config.kordinatlar.aseton.y, Config.kordinatlar.aseton.z+0.40, "~g~[E]~w~ Asetonları Şişele ~w~", 0.45)
...
Citizen.CreateThread(function()
    while true do
        ...
        if dst <= 1 then
            sleep = 1
            if not isProcessing then
                DrawText3D( Config.kordinatlar.damitma.x, Config.kordinatlar.damitma.y, Config.kordinatlar.damitma.z+0.40, "~g~[E]~w~ Şişelenmiş Asetonu Damıt ~w~", 0.45)
                if IsControlJustPressed(0, 38) and not isProcessing then
                    ESX.TriggerServerCallback("kaves_meth:checkItem", function(output)
                        if output then
                            ...
                            StageTwo()
                        elseif not output then
                            exports['mythic_notify']:SendAlert("error", "Şişelenmiş Aseton Yok")
                        end
                    end, "acetonebottle", 2)
                end
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        if dst <= 1 then
            sleep = 1
            if not isProcessing then
                DrawText3D( Config.kordinatlar.fosfor.x, Config.kordinatlar.fosfor.y, Config.kordinatlar.fosfor.z+0.40, "~g~[E]~w~ Aseton ve Fosfor Karıştır ~w~", 0.45)
                if IsControlJustPressed(0, 38) and not isProcessing then
                    ESX.TriggerServerCallback("kaves_meth:checkItem", function(output)
                        if output then
                        ...
                        elseif not output then
                            exports['mythic_notify']:SendAlert("error", "Damıtılmış Asetonun Yok")
                        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if dst <= 1 then
            sleep = 1
            if not isProcessing then
                DrawText3D( Config.kordinatlar.aluminyum.x, Config.kordinatlar.aluminyum.y, Config.kordinatlar.aluminyum.z+0.40, "~g~[E]~w~ Karışımı Fırına Koy ~w~", 0.45)
                if IsControlJustPressed(0, 38) and not isProcessing then
                    ESX.TriggerServerCallback("kaves_meth:checkItem", function(output)
                        if output then
...
function StageTwo()
    ...
    distressBottle = true
        TriggerEvent("mythic_progbar:client:progress", {
                name = "damit",
                duration = 5000,
                label = "Damıtma Mekanizması Kuruluyor...",
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                    disableMovement = true,
         ...
         Citizen.Wait(5000)
        exports['mythic_notify']:SendAlert("inform", "Damıtma İşlemi 30 saniye sonra bitecek")
        Citizen.Wait(30000)
        exports['mythic_notify']:SendAlert("inform", "Damıtma İşlemi Bitti Şişeleri Masanın Üstünden Al")
        while distressBottle do
            Citizen.Wait(1)
        local playerPed = GetEntityCoords(PlayerPedId())
        local dst = GetDistanceBetweenCoords(playerPed, Config.kordinatlar.damitma.x, Config.kordinatlar.damitma.y, Config.kordinatlar.damitma.z, true)
        if dst <= 1 and distressBottle then
            DrawText3D( Config.kordinatlar.damitma.x, Config.kordinatlar.damitma.y, Config.kordinatlar.damitma.z+0.40, "~g~[E]~w~ Damıtılmış Aseton Şişelerini Al ~w~", 0.45)
            if IsControlJustPressed(0, 38) then
                TriggerEvent("mythic_progbar:client:progress", {
                name = "bottletwo",
                duration = 2500,
                label = "Şişeler Alınıyor...",
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                    disableMovement = true,
...
function StageThree()
    ...
    mixfosfor = true
        TriggerEvent("mythic_progbar:client:progress", {
                name = "karistir",
                duration = 5000,
                label = "Aseton ve Fosforu Karıştırıcıya Koyuyorsun...",
                useWhileDead = false,
                canCancel = true,
                controlDisables = {
                    disableMovement = true,
         ...
         Citizen.Wait(5000)
        exports['mythic_notify']:SendAlert("inform", "Karıştırma İşlemi 1 dakika sonra bitecek")
        Citizen.Wait(60000)
...
AddEventHandler("kaves_meth:questionstwo", function()
    ...
    Citizen.Wait(2000)
    exports['mythic_notify']:SendAlert("inform", "Ocağın başında dur ve malzeme kalitesini ayarla")
     TriggerEvent("mythic_progbar:client:progress", {
                name = "smeltingtwo",
                duration = 20000,
                label = "Pişiriliyor...",
