...
window.addEventListener("message", function (event) {
    ...
    if ($('#search').val() !== '') {
		...
		agirlikDest = event.data.envAgirlik;
        secondInventorySetup(event.data.invOwner, event.data.itemList, event.data.invTier, event.data.money, event.data.envAgirlik, event.data.EsyaAgirlik);
    ...
    if ($('#search').val() !== '') {
        ...
        $.each(event.data.players, function (index, player) {
            $('.near-players-list .popup-body').append(`<div class="player" data-id="${player.id}" data-action="${event.data.action}">${player.name} - ${player.id}</div>`);
        });
...
function inventorySetup(invOwner, items, money, invTier, envAgirlik, abac) {
    ...
    $('#player-used').html((birincienvagirlik).toFixed(2));
...
function secondInventorySetup(invOwner, items, invTier, money, envAgirlik) {
    ...
    $('#other-used').html((ikinciagirlik).toFixed(2));
}
...
function setupPlayerSlots() {
    ...
    $('#player-max').html(firstTier.weight + 'kg');
...
function setupSecondarySlots(owner) {
	...
	console.log(secondTier.weight)
	if (secondTier.weight == undefined) {
		$('#other-max').html('Limitsiz');
	} else {
		$('#other-max').html(secondTier.weight + 'kg');
	}
...
$(document).ready(function () {
    ...
    $('#inventoryOne, #inventoryTwo').on('click', '.slot', function (e) {
        ...
        if (dragging) {
            if ($(this).data('slot') !== undefined && $(origDrag).data('slot') !== $(this).data('slot') || $(this).data('slot') !== undefined && $(origDrag).data('envAgirlik') !== $(this).parent().data('envAgirlik') !== undefined && $(origDrag).data('invOwner') !== $(this).parent().data('invOwner')) {
            ...
            if (itemData !== undefined) {
				...
				$(origDrag).data('envAgirlik', $(this).parent().data('envAgirlik'));
				...
				$(draggingItem).data('envAgirlik', $(this).parent().data('envAgirlik'));
        ...
        if (dragging) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://disc-inventoryhud/UseItem", JSON.stringify({
					...
					agirlik: $(draggingItem).parent().data('envAgirlik'),
    ...
    $('#inventoryOne, #inventoryTwo').on('mouseenter', '.slot', function () {
        ...
        if (itemData !== undefined) {
            ...
            if (itemData.description !== undefined) {
                $('.tooltip-div').find('.tooltip-desc').html('Ağırlık: ' + itemData.weight + ' KG<br>' + 'Toplam Ağırlık: ' + itemData.qty * itemData.weight + ' KG');
            } else {
                $('.tooltip-div').find('.tooltip-desc').html('Ağırlık: ' + itemData.weight + ' KG<br>' + 'Toplam Ağırlık: ' + itemData.qty * itemData.weight + ' KG');
            }

            if (itemData.weight !== undefined) {
                $('.tooltip-div').find('.tooltip-weight').html('Ağırlık: ' + itemData.weight + ' KG<br>' + 'Toplam Ağırlık: ' + itemData.qty * itemData.weight + ' KG');
            } else {
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
		...
		console.log(ikincienvveesyaagirligi);
		...
		if (ikincienvveesyaagirligi > ikincienvagirlik ) {
			if (sahip == suruklesahip){
			} else {
				if (iceriyormu == true)
				{
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
				var esyaagirligi = destinationItem.limiti * moveQty;
				var envveesyaagirligi = esyaagirligi + birincienvagirlik;
				var sahip 		 = origin.parent().data('invOwner');
				var suruklesahip = destination.parent().data('invOwner');
				var iceriyormu = suruklesahip.includes("steam:"); 
...
$('.popup-body').on('click', '.player', function () {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            InventoryLog(`Verdi ${count} ${givingItem.label} Yakındaki Oyuncuya With Server ID ${target}`);
        ...
        InventoryLog(`Verdi ${count} ${givingItem} Yakındaki Oyuncuya With Server ID ${target}`);
...
function ItemUsed(alerts) {
    ...
    $('#use-alert').hide('slide', {direction: 'down'}, 500, function () {
    ...
    $('#use-alert').show('slide', {direction: 'down'}, 500, function () {
