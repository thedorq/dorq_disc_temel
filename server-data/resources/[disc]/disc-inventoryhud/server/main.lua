...
RegisterServerEvent('disc-inventoryhud:yenile')
AddEventHandler('disc-inventoryhud:yenile', function()
    local owner = ESX.GetPlayerFromId(destinationPlayer).identifier
    MySQL.Async.fetchAll('DELETE FROM disc_inventory WHERE data = @data AND owner = @owner', { ['@data'] = "null", ['@owner'] = owner })  -- Tgiann "Null" Fix
    ensureInventories(destinationPlayer)
end)
RegisterCommand('envkon', function(source)
...
RegisterServerEvent('disc-inventoryhud:ensure')
AddEventHandler('disc-inventoryhud:ensure', function()
    local owner = ESX.GetPlayerFromId(destinationPlayer).identifier
